<template>
    <div class="tab-body">
                ...
                <el-row>
                    ...
                    <el-col :span="4">
                        <el-form-item label="考试类型">
                            <select-option-dictionary v-model="searchParams.extype" @change="search()" dkey="exp-type">
        ...
        <div class="dataTable autoFlex" v-if="listData">
            <el-table class="table" :data="listData" stripe border height="auto" @selection-change="selectionChange"
                ...
                <el-table-column label="得分/及格分/总分">
                    <template slot-scope="scope">
                        <el-col>
                            <el-tag type="danger" v-if="scope.row.anscore < scope.row.passScore">{{ scope.row.anscore }}
                            </el-tag>
                            <el-tag v-if="scope.row.anscore > scope.row.passScore">{{ scope.row.anscore }}</el-tag>
                            / <el-tag type="info">{{ scope.row.passScore }}</el-tag>
                            / <el-tag type="info">{{ scope.row.exscores }}</el-tag>
                        </el-col>
                    </template>
                </el-table-column>
                <el-table-column label="及格人数/考试人数">
                    <template slot-scope="scope">
                        <el-col>
                            <el-tag v-if="scope.row.jige">{{ scope.row.jige }}</el-tag>
                            / <el-tag type="info" v-if="scope.row.couex">{{ scope.row.couex }}</el-tag>
                        </el-col>
                    </template>
                </el-table-column>
                <el-table-column label="平均分/最高分">
                    <template slot-scope="scope">
                        <el-col>
                            <el-tag v-if="scope.row.avgscore">{{ scope.row.avgscore }}</el-tag>
                            / <el-tag type="info" v-if="scope.row.maxscore">{{ scope.row.maxscore }}</el-tag>
                        </el-col>
                    </template>
                </el-table-column>
                    ...
                    <template slot-scope="scope">
                        <div class="dataTable-operator">
                            <el-button @click="toHistory(scope.$index, scope.row)"
                                v-bind:disabled="scope.row.lookState == 0 ? true : false">查 看</el-button>
                        </div>
                    </template>
                </el-table-column>
            </el-table>
...
<script>
import startTest from "../exams/startTest.vue";
import edit from "../exams/edit.vue";
